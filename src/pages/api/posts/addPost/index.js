      ...
      if (images[0] === undefined) {
        ...
        const movePromises = images.map(async (image) => {
          if (image.newFilename) {
            return moveVideo(image, fields);
          } else {
            return insertVideo(fields.oldimage, fields);
          }
        });
        ...
        try {
          ...
          console.error(error);
          res.status(500).json({ error: 'Une erreur s\'est produite lors de l\'enregistrement des vidéos.', details: error.message });
        }
      }
    });
  } else {
...
async function insertVideo(image, fields) {
  ...
  try {
    ...
    console.error(error);
    // Gérer l'erreur, par exemple, renvoyer une réponse d'erreur au client
